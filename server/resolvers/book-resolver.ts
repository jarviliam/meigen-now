...
export class BookResolver {
    ...
    async getBookById(@Arg("id") id : string) : Promise<Book>{
        return await FbApp().collection('books').doc(id).get()
        .then((response : any) => {
            if (response){
    ...
    async getAllBooks() : Promise<Array<Book>> {
        return await FbApp().collection('books').get()
        .then((result: any) => {
            const bookArray: Array<Book> = [] 
            result.map((book : any) => {
                //TODO: Hook up author ID connection
                let bookObject = plainToClass(Book, {id: book.id, title: book.data().title, publishedDate: book.data().publishedDate})
